"""
Implementation of Q-scores adapted from https://github.com/facebookresearch/PoincareMaps
Includes worst case distortion and MAP score in addition
"""
...
def compute_MAP_score(coord_high, Rank_low, k=1):
    KNN = kneighbors_graph(coord_high, k, mode='connectivity', include_self=False).toarray()
    num_nodes = KNN.shape[0]
    # initialize MAP score
    MAP_score = 0
    for u in range(num_nodes):
        ranks = Rank_low[u] * KNN[u]
        # Rs are embedding-space ranks of 1-hop neighbors of node u in KNN
        # Rs = ranks[np.nonzero(ranks)]
        # these are 1-hop neighbors of node u
        N1 = np.nonzero(KNN[u])[0]
        Rs = ranks[N1]
        quotient = 0
        for R in Rs:
            # R_ui is the embedding-space neighborhood containing the ith neighbor of node u
            R_ui = np.where(Rank_low[u] <= R)[0]
            quotient += (1. / len(N1)) * len(np.intersect1d(N1, R_ui)) / len(R_ui)
        MAP_score += quotient
    return MAP_score / num_nodes
...
def get_quality_metrics(
    ...
    inds = np.triu_indices(n=len(D_high), k=1)
    D_low = scale_distance(D_low)
    D_high = scale_distance(D_high)
    dist_num = ((D_low[inds] / D_high[inds])**2 -1)
    distortion = dist_num.ravel()

    # compute MAP-1 score, i.e. for nearest neighbor in D_high (based on KNN)
    # find the smallest set of nearest neighbors in D_low that contain the nearest neighbor
    # and compute the statistic as in M. Weber's supplement

    map_score = compute_MAP_score(coord_high=coord_high, Rank_low=Rank_low, k=map_k)
    print(f"Worst case Distortion = {np.abs(distortion).max():.2f}, map score = {map_score: .2f}")

    return Qlocal, Qglobal, Kmax, distortion, map_score
